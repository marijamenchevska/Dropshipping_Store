export class ProductFunctionality {
  static loadFavourites(productsArray) {
    ...
    const favItems = JSON.parse(favesLocalStorage);

    for (let favItem of favItems) {
      const productIndex = productsArray.findIndex(
        (product) => product.id === favItem.id
      );
  ...
  static toggleFavourites(productObject, favBtn) {
    ...
    const itemIndex = favItems.findIndex(
    ...
    if (itemIndex === -1) {
      ...
      delete productObject.favourited;
  ...
  static syncFavsOnTwoPages(oldFavesArray, newFavesArray, productsArray) {
    ...
    if (!oldFavesArray) {
      ...
      productIndex = productsArray.findIndex(
      ...
      const likedProduct = newFavesArray.find(
        (product) => !oldFavesArray.find((item) => item.id === product.id)
      );
      ...
      const unlikedProduct = oldFavesArray.find(
        (product) => !newFavesArray.find((item) => item.id === product.id)
      );

      productIndex = productsArray.findIndex(
        (product) => product.id === unlikedProduct.id
      );

      delete productsArray[productIndex].favourited;
    }
