...
export class CategoryProdService {
  constructor(categoryInfoObject) {
    ...
    this.categoryPage = document.getElementById("category-page");
    ...
    this.categoryPageContainer = document.getElementById(
  ...
  createAnyCategoryPage() {
    this.currentProducts = this.allProducts.slice(
      this.firstProductIndex,
      this.lastProductIndex
    );

    ProductFunctionality.loadFavourites(this.currentProducts);
  ...
  categoryEvents() {
    ...
    // We always go back to the first page - keep the changes from Sort by
    this.showItems.forEach((item) => {
      item.addEventListener("click", (e) => {
        ...
        // This is needed in case the number of products per page = total number of products
    ...
    // We always go back to the first page - keep the changes from Show
